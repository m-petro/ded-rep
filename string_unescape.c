...
/*@
	axiomatic len_of_hex 
	{
		...
		axiom L1: \forall char *x; *x == 'x' &&  isxdigit(*(x + 1)) && !isxdigit(*(x + 2)) 	==> len_of_hex(x) ==  2;
		axiom L2: \forall char *x; *x == 'x' &&  isxdigit(*(x + 1)) && isxdigit(*(x + 2)) 	==> len_of_hex(x) ==  3;
	}
...
/*@
	requires \valid (src) && \valid (dst) && \valid (*src) && \valid(*dst);
	assigns (**dst), *src, *dst;
	behavior is_space:

...
static bool unescape_octal(char **src, char **dst)
{
	...
	// loop variant (q - *src); - оба варианта не доказываются
	 loop variant (*src - q); 
	*/
...
/*@ requires \valid(src + (0..size - 1));
	...
	//behavior is_hex:
	//assumes
	//	(flags & (unsigned int)UNESCAPE_HEX) != 0; - непонятно, как отсчитывать количество измененных символов в каждой вложенной функции
